    ...
    public static boolean importDefinitions(String className, ModuleInfo info, Procedure renamer, Vector forms, ScopeExp defs, Compilation tr) {
        ...
        if (isRunnable) {
            ...
            if (decl != null) {
                ...
                dofind = new ApplyExp((Procedure) SlotGet.staticField, new Expression[]{new QuoteExp(type), new QuoteExp("$instance")});
            }
            ApplyExp applyExp = new ApplyExp(run, new Expression[]{dofind});
            applyExp.setLine(tr);
