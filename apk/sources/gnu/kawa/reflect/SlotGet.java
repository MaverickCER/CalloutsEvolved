...
public class SlotGet extends Procedure2 implements HasSetter, Inlineable {
    ...
    public Type getReturnType(Expression[] args) {
        if (args.length == 2) {
            ...
            if (quoteExp instanceof QuoteExp) {
                ...
                if (!this.isStatic && (arg0.getType() instanceof ArrayType) && "length".equals(ClassMethods.checkName(quoteExp, true))) {
    ...
    public static ApplyExp makeGetField(Expression value, String fieldName) {
        return new ApplyExp((Procedure) field, new Expression[]{value, new QuoteExp(fieldName)});
    }
}
