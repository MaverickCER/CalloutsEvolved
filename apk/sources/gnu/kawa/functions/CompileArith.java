...
public class CompileArith implements Inlineable {
    ...
    public static CompileArith forDiv(Object proc2) {
        return new CompileArith(proc2, ((DivideOp) proc2).f23op);
    }

    public static CompileArith forBitwise(Object proc2) {
        return new CompileArith(proc2, ((BitwiseOp) proc2).f23op);
    }
    ...
    public void compile(ApplyExp exp, Compilation comp, Target target) {
        ...
        if (len == 0) {
            ...
            int kind1 = Arithmetic.classifyType(args[0].getType());
            int kind2 = Arithmetic.classifyType(args[1].getType());
            int kind = getReturnKind(kind1, kind2, this.f24op);
            ...
            if (this.f24op >= 4 && this.f24op <= 8) {
                DivideOp dproc = (DivideOp) this.proc;
                if (dproc.f23op != 4 || (kind > 4 && kind < 6 && kind > 9)) {
                    if ((dproc.f23op == 5 && kind <= 10 && kind != 7) || (dproc.f23op == 4 && kind == 10)) {
                        kind = 8;
                    } else if (((dproc.f23op != 7 && (dproc.f23op != 6 || kind > 4)) || !(dproc.getRoundingMode() == 3 || kind == 4 || kind == 7 || kind == 8)) && !(dproc.f23op == 8 && (dproc.getRoundingMode() == 3 || kind == 4))) {
                        ApplyExp.compile(exp, comp, target);
                        return;
                    }
                }
            }
            if (this.f24op == 4 && kind <= 10 && kind != 8 && kind != 7) {
                if (kind == 6 || kind > 4) {
            ...
            } else if (kind == 4 && (this.f24op == 1 || this.f24op == 3 || this.f24op == 2 || this.f24op == 13 || this.f24op == 14 || this.f24op == 15 || this.f24op == 7 || this.f24op == 8 || (this.f24op >= 9 && this.f24op <= 11))) {
                compileIntNum(args[0], args[1], kind1, kind2, comp);
            } else if (kind == 1 || kind == 2 || ((kind == 7 || kind == 8) && (this.f24op <= 8 || this.f24op >= 13))) {
                Target wtarget2 = StackTarget.getInstance(wtype2);
                CodeAttr code = comp.getCode();
                for (int i = 0; i < len; i++) {
                    if (i == 1 && this.f24op >= 9 && this.f24op <= 12) {
    ...
    public Type getReturnType(Expression[] args) {
        return Arithmetic.kindType(adjustReturnKind(getReturnKind(args), this.f24op));
    }
    ...
    public static Expression validateApplyNot(ApplyExp exp, int kind, InlineCalls visitor) {
        ...
        if (kind == 1 || kind == 2) {
            return visitor.visitApplyOnly(new ApplyExp((Procedure) BitwiseOp.xor, new Expression[]{arg, QuoteExp.getInstance(IntNum.minusOne())}), (Type) null);
        }
        if (kind == 4) {
    ...
    public static Expression pairwise(Procedure proc2, Expression rproc, Expression[] args, InlineCalls visitor) {
        ...
        for (int i = 1; i < len; i++) {
            ApplyExp next = new ApplyExp(rproc, new Expression[]{prev, args[i]});
    ...
    public static Expression validateApplyNumberPredicate(ApplyExp exp, InlineCalls visitor, Type required, Procedure proc2) {
        int i = ((NumberPredicate) proc2).f28op;
        Expression[] args = exp.getArgs();
