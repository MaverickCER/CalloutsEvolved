...
public class CompileNamedPart {
    ...
    public static Expression makeGetNamedInstancePartExp(Expression member) {
        ...
        return new ApplyExp((Procedure) Invoke.make, new Expression[]{new QuoteExp(ClassType.make("gnu.kawa.functions.GetNamedInstancePart")), member});
    }
    ...
    public static Expression validateSetNamedInstancePart(ApplyExp exp, InlineCalls visitor, Type required, Procedure proc) {
        exp.visitArgs(visitor);
        Expression[] args = exp.getArgs();
        return visitor.visitApplyOnly(new ApplyExp((Procedure) SlotSet.set$Mnfield$Ex, new Expression[]{args[0], new QuoteExp(((SetNamedInstancePart) proc).pname), args[1]}), required);
    }
}
