...
public class XslTranslator extends Lexer implements Consumer {
    ...
    public void endAttribute() {
        push((Expression) new ApplyExp((Expression) MakeAttribute.makeAttributeExp, new Expression[]{new QuoteExp(this.attributeType), new QuoteExp(this.attributeValue.toString())}));
    ...
    public void endElement() {
        ...
        if (xslTag == "value-of") {
            ...
            if (select != null) {
                Expression exp = new ApplyExp(XQParser.makeText, new Expression[]{parseXPath(select)});
            ...
            this.comp.exprStack.pop();
            push((Expression) new ApplyExp((Expression) new QuoteExp(defineTemplateProc), new Expression[]{resolveQNameExpression(name), new QuoteExp(match), new QuoteExp(DFloNum.make(0.0d)), resolveQNameExpression(mode), this.templateLambda}));
    ...
    public void parse(Compilation comp2) throws IOException {
        ...
        startDocument(mexp2);
        XMLParser.parse((LineBufferedReader) this.f200in, getMessages(), (Consumer) this);
        endDocument();
        comp2.pop(mexp2);
    }
}
