...
public class CompileMisc {
    ...
    public static Expression validateApplyValuesFilter(ApplyExp exp, InlineCalls visitor, Type required, Procedure proc) {
        ...
        if (vproc.kind == 'P') {
            ...
            ApplyExp applyExp = new ApplyExp((Procedure) SortNodes.sortNodes, new Expression[]{seq});
        ...
        if (vproc.kind == 'R') {
            ...
            Expression init = new ApplyExp((Procedure) AddOp.$Mn, new Expression[]{new ReferenceExp(lastArg), new ReferenceExp(declaration)});
            LetExp let = new LetExp(new Expression[]{new ApplyExp((Procedure) AddOp.$Pl, new Expression[]{init, new QuoteExp(IntNum.one())})});
        ...
        ApplyExp doMap = new ApplyExp((Procedure) ValuesMap.valuesMapWithPos, new Expression[]{lexp2, new ReferenceExp(sequence)});
        doMap.setType(dotArg.getType());
        lexp2.returnContinuation = doMap;
        LetExp let2 = new LetExp(new Expression[]{new ApplyExp(sizeMethod, new Expression[]{new ReferenceExp(sequence)})});
        let2.add(lastArg);
    ...
    public static Expression validateApplyOrderedMap(ApplyExp exp, InlineCalls visitor, Type required, Procedure proc) {
        ...
        return new ApplyExp(proc, new Expression[]{args[0], new ApplyExp(typeTuples.getDeclaredMethod("make$V", 2), rargs)});
    }
