...
public class XQParser extends Lexer {
    ...
    public static final Convert treatAs = Convert.f25as;
                ...
                switch (ch) {
                    ...
                    default:
                        if (!Character.isDigit(ch) && (ch != '.' || !Character.isDigit((char) peek()))) {
                            ...
                            if (next != 101 && next != DEFAULT_ELEMENT_TOKEN) {
        ...
        if (this.curToken == NCNAME_TOKEN) {
            switch (this.tokenBufferLength) {
                case 2:
                    ...
                    if (c1 != DECLARE_OPTION_TOKEN || c2 != 'r') {
                        if (c1 != 't' || c2 != DECLARE_OPTION_TOKEN) {
                            if (c1 != 'i' || c2 != 's') {
                                if (c1 != 'e' || c2 != 'q') {
                                    if (c1 != 'n' || c2 != 'e') {
                                        if (c1 != 'g') {
                ...
                case 3:
                    ...
                    if (c12 != 'a') {
                        if (c12 != 'm') {
                            ...
                            if (c22 == DECLARE_OPTION_TOKEN && c3 == 'd') {
                                this.curToken = 418;
                                break;
                            }
                        }
    ...
    public Expression parseDataType() throws IOException, SyntaxException {
        ...
        getRawToken();
        ApplyExp otype = new ApplyExp((Procedure) proc_OccurrenceType_getInstance, new Expression[]{etype, QuoteExp.getInstance(IntNum.make(min)), QuoteExp.getInstance(IntNum.make(max))});
        otype.setFlag(4);
        return otype;
    }
    ...
    public Expression parseUnaryExpr() throws IOException, SyntaxException {
        ...
        getRawToken();
        return new ApplyExp(makeFunctionExp("gnu.xquery.util.ArithOp", op == 413 ? "plus" : "minus", op == 413 ? "+" : "-"), new Expression[]{parseUnaryExpr()});
    }
    ...
    public Expression parseNameTest(boolean attribute) throws IOException, SyntaxException {
        ...
        args[0] = new ApplyExp((Expression) new ReferenceExp(XQResolveNames.resolvePrefixDecl), new Expression[]{QuoteExp.getInstance(prefix)});
        if (local == null) {
            str = "";
        } else {
            str = local;
    ...
    public Expression parseNodeTest(int axis) throws IOException, SyntaxException {
        ...
        mkAxis.setFlag(4);
        return new ApplyExp((Expression) mkAxis, new Expression[]{dot});
    }
    ...
    public Expression parseRelativePathExpr(Expression exp) throws IOException, SyntaxException {
        ...
        while (true) {
            ...
            if (descendants) {
                this.curToken = 47;
                lexp.body = new ApplyExp((Procedure) DescendantOrSelfAxis.anyNode, new Expression[]{new ReferenceExp(DOT_VARNAME, dotDecl)});
            ...
            this.comp.pop(lexp);
            exp = new ApplyExp((Procedure) RelativeStep.relativeStep, new Expression[]{exp, lexp});
        }
    }
    ...
    public Expression parseStepExpr() throws IOException, SyntaxException {
        ...
        if (this.curToken == 46 || this.curToken == DOTDOT_TOKEN) {
            ...
            if (axis == 9) {
                exp = new ApplyExp((Procedure) ParentAxis.make(NodeType.anyNodeTest), new Expression[]{exp});
            }
            if (axis == 12) {
                i = -1;
            } else {
    ...
    public Expression parseXMLConstructor(int next, boolean inElementContent) throws IOException, SyntaxException {
        if (next == 33) {
            ...
            if (next2 == 45 && peek() == 45) {
                ...
                return new ApplyExp(makeFunctionExp("gnu.kawa.xml.CommentConstructor", "commentConstructor"), new Expression[]{new QuoteExp(new String(this.tokenBuffer, 0, this.tokenBufferLength))});
            } else if (next2 != 91 || read() != NCNAME_COLON_TOKEN || read() != SLASHSLASH_TOKEN || read() != NCNAME_TOKEN || read() != IMPORT_SCHEMA_TOKEN || read() != NCNAME_TOKEN || read() != 91) {
                ...
                getDelimited("]]>");
                return new ApplyExp(makeCDATA, new Expression[]{new QuoteExp(new String(this.tokenBuffer, 0, this.tokenBufferLength))});
            }
        } else if (next == 63) {
            int next3 = peek();
            if (next3 < 0 || !XName.isNameStart((char) next3) || getRawToken() != NCNAME_TOKEN) {
            ...
            return new ApplyExp(makeFunctionExp("gnu.kawa.xml.MakeProcInst", "makeProcInst"), new Expression[]{new QuoteExp(target), new QuoteExp(new String(this.tokenBuffer, 0, this.tokenBufferLength))});
    ...
    static ApplyExp castQName(Expression value, boolean element) {
        return new ApplyExp((Expression) new ReferenceExp(element ? XQResolveNames.xsQNameDecl : XQResolveNames.xsQNameIgnoreDefaultDecl), new Expression[]{value});
    }
    ...
    public Expression wrapWithBaseUri(Expression exp) {
        ...
        return new ApplyExp((Procedure) MakeWithBaseUri.makeWithBaseUri, new Expression[]{new ApplyExp((Expression) new ReferenceExp(XQResolveNames.staticBaseUriDecl), Expression.noExpressions), exp}).setLine(exp);
    }
    ...
    public boolean match(String word) {
        int len;
        if (this.curToken != NCNAME_TOKEN || this.tokenBufferLength != (len = word.length())) {
            return false;
        }
        int i = len;
        do {
