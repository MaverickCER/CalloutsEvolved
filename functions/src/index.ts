...
async function getSubscriptionData(subscriptionId: string): Promise<any> {
  const response = await axios
    .get(`https://api-m.paypal.com/v1/billing/subscriptions/${subscriptionId}`, {
      auth,
      headers,
    })
    .catch((error: any) => error.toJSON());

  return response;
}

async function cancelSubscriptionData(subscriptionId: string, reason: string): Promise<any> {
  const response = await axios
    .post(
...
exports.getSubscription = functions.https.onCall(
  async (data: { subscriptionId: string }, context: { auth: { uid: string } }) => {
...
exports.cancelSubscription = functions.https.onCall(
  async (data: { subscriptionId: string; reason: string }, context: { auth: { uid: string } }) => {
    let subscriptionId = data.subscriptionId;
    ...
    let error: object = { error: 'Missing subscriptionId' };
